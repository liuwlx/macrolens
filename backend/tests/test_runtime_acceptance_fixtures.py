...
def test_clean_seed_mappings_receive_fixture_probe_approval(
    ...
    assert all(job.result.get("request_url") == "fixture://runtime-acceptance" for job in jobs)
    assert all(job.result.get("content_type") == "application/json" for job in jobs)
    assert all(job.result.get("official_description") for job in jobs)
    assert all(job.result.get("probed_at") for job in jobs)
    assert all(job.result.get("evidence", {}).get("details", {}).get("fixture") for job in jobs)
