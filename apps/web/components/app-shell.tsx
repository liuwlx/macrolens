  ...
  return (
    ...
    >
      <div className="flex h-[68px] items-center gap-3 border-b border-slate-200 px-5">
        ...
        {mobile && <button aria-label="关闭导航" className="ml-auto btn btn-ghost !min-h-8 !px-2" onClick={onCloseMobile}><X size={18} /></button>}
      </div>
      ...
      {!mobile && (
        <button className="flex h-12 items-center gap-3 border-t border-slate-200 px-6 text-sm text-slate-500 hover:bg-slate-50" onClick={onToggleCollapsed}>
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#f5f7fb]">
      <Sidebar collapsed={collapsed} navItems={navItems} pathname={pathname} onCloseMobile={() => setMobileOpen(false)} onToggleCollapsed={() => setCollapsed(!collapsed)} />
      {mobileOpen && <><div className="fixed inset-0 z-40 bg-slate-950/30 md:hidden" onClick={() => setMobileOpen(false)} /><Sidebar mobile collapsed={collapsed} navItems={navItems} pathname={pathname} onCloseMobile={() => setMobileOpen(false)} onToggleCollapsed={() => setCollapsed(!collapsed)} /></>}
      <div className={`transition-all ${collapsed ? "md:ml-[76px]" : "md:ml-[222px]"}`}>
        ...
        <main className="min-w-0 w-full p-4 pb-24 md:p-6 md:pb-8">{children}</main>
      </div>
