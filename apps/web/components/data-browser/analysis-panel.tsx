...
export function AnalysisPanel({ state, item, analytics, analyticsLoading, analyticsError, onChange, onRetryAnalytics }: Props) {
  ...
  const observations = useQuery({
    ...
    enabled: Boolean(selectedId) && (state.tab === "trend" || state.tab === "history"),
    staleTime: 5 * 60_000,
    retry: (count, error) => !("status" in (error as object)) && count < 1,
  });
  const revisions = useQuery({ queryKey: ["data-browser-revisions", selectedId, state.data_as_of], queryFn: ({ signal }) => apiFetch<RevisionResponse>(`/series/${selectedId}/revisions${queryString({ data_as_of: state.data_as_of })}`, { signal }), enabled: Boolean(selectedId) && state.tab === "revisions", staleTime: 5 * 60_000 });
  ...
  return <section className="data-browser-card data-browser-analysis-card" id="data-browser-analysis" aria-labelledby="analysis-title">
    ...
    <div className="data-browser-analysis-content" role="tabpanel" aria-labelledby="analysis-title">
      ...
      {selectedId && state.tab === "trend" && <div className="data-browser-trend-grid">
