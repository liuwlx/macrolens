...
export default function ReportsPage() {
  ...
  const [editedReportId, setEditedReportId] = useState("");
  ...
  const titleValue = createMode || editedReportId === selected?.id ? title : selected?.title ?? "";
  const bodyValue = createMode || editedReportId === selected?.id ? body : selected?.content_markdown ?? "";
  ...
  const createReport = useMutation({
    mutationFn: () => {
      ...
      return apiFetch<Report>("/me/reports", {
        method: "POST",
        body: JSON.stringify({
          title: titleValue || run?.prompt.slice(0, 120) || "MacroLens研究报告",
          content_markdown: bodyValue || null,
  ...
  const saveReport = useMutation({
    mutationFn: (status?: string) => apiFetch<Report>(`/me/reports/${selected!.id}`, {
      method: "PATCH",
      body: JSON.stringify({ title: titleValue, content_markdown: bodyValue, status: status ?? selected!.status }),
    }),
  ...
  function download(format: "md" | "html") {
    ...
    const markdown = `# ${titleValue}\n\n${bodyValue}\n\n## 来源引用\n\n${source}`;
    const escapeHtml = (value: string) => value.replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll('"', "&quot;").replaceAll("'", "&#39;");
    const escaped = escapeHtml(markdown);
    const html = `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><title>${escapeHtml(titleValue)}</title><style>body{font-family:system-ui;margin:48px auto;max-width:900px;line-height:1.8;color:#172033}pre{white-space:pre-wrap}</style></head><body><pre>${escaped}</pre></body></html>`;
  ...
  return <div>
    ...
    {!reports.data?.length ? <EmptyState title="暂无持久化报告" description="选择一条已完成AI分析，或手工输入内容创建第一份报告。"/> : <div className="grid gap-5 xl:grid-cols-[300px_minmax(0,1fr)_330px]">
      ...
      <main className="card min-w-0"><div className="card-header"><div><h2 className="section-title">报告编辑器</h2><p className="mt-1 text-xs text-slate-500">正文变化会增加版本号</p></div><div className="flex gap-2"><button className="btn" disabled={saveReport.isPending} onClick={()=>saveReport.mutate(undefined)}><Save size={15}/>保存草稿</button><button className="btn btn-danger" onClick={()=>{if(confirm("确认删除该报告？"))deleteReport.mutate()}}><Trash2 size={15}/></button></div></div><div className="card-body"><input className="input mb-3 text-lg font-bold" value={titleValue} onChange={(event)=>{setEditedReportId(selected!.id);setTitle(event.target.value)}}/><textarea className="textarea min-h-[610px] font-mono text-sm leading-7" value={bodyValue} onChange={(event)=>{setEditedReportId(selected!.id);setBody(event.target.value)}}/></div></main>
