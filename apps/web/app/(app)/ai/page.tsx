...
import { Bot, CheckCircle2, FileText, LoaderCircle, Play, Plus, Send, Sparkles, Trash2, X } from "lucide-react";
...
import type { AICitation, AIRun, DocumentSummary, Note, SavedView, SeriesSummary } from "@/lib/types";
...
function AiContent() {
  ...
  const [prompt, setPrompt] = useState("请结合最新通胀、就业和FOMC材料，分析未来两次会议的政策路径，并区分事实、推断与风险情景。");
  ...
  useEffect(()=>{ if (!selectedRunId && runsQuery.data?.[0]) setSelectedRunId(runsQuery.data[0].id); },[runsQuery.data,selectedRunId]);
  useEffect(()=>{const runId=searchParams.get("run");if(runId&&runId!==selectedRunId)setSelectedRunId(runId)},[searchParams,selectedRunId]);
  const selectedRun = runsQuery.data?.find((item)=>item.id===selectedRunId);
  const citationsQuery = useQuery({ queryKey:["ai-citations",selectedRunId], queryFn:()=>apiFetch<AICitation[]>(`/ai/runs/${selectedRunId}/citations`), enabled:Boolean(selectedRunId)&&selectedRun?.status==="completed" });
  ...
  useEffect(()=>{
    ...
    if (seriesId) setContexts((prev)=>prev.some((item)=>item.context_id===seriesId)?prev:[...prev,{context_type:"series",context_id:seriesId,label:"已选指标"}]);
    if (documentId) setContexts((prev)=>prev.some((item)=>item.context_id===documentId)?prev:[...prev,{context_type:"document",context_id:documentId,label:"已选文档"}]);
  },[searchParams]);

  const createRun = useMutation({
    mutationFn:()=>apiFetch<AIRun>("/ai/runs",{method:"POST",body:JSON.stringify({prompt,mode,contexts:contexts.map(({context_type,context_id})=>({context_type,context_id}))})}),
