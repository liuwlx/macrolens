...
describe("TradingView sync error messages", () => {
  it("turns a reset during the websocket handshake into an actionable message", () => {
    expect(formatTradingViewSyncError("ProviderDataError: TradingView WebSocket connection failed: the remote server or outbound proxy reset the connection")).toBe(
